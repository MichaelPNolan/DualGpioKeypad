/*
||
|| @file Keypad_MC17b.cpp
|| @version 2.0
|| @author G. D. (Joe) Young
|| @contact "G. D. (Joe) Young" <jyoung@islandnet.com>
||
|| @description
|| | Keypad_MC17b provides an interface for using matrix keypads that
...
void Keypad_MC17b::begin(char *userKeymap) {
...
// Initialize MC17
void Keypad_MC17b::begin(void) {
	_begin( );
	pinState = pinState_set( );
}

word iodirec_b = 0xffff;            //direction of each bit - reset state = all inputs.
byte iocon_b = 0x10;                // reset state for bank, disable slew control

// Initialize MCP23017
// MCP23017 byte registers act in word pairs

void Keypad_MC17b::_begin( void ) {
	iodir_state = iodirec_b;
	...
	_wire->write( lowByte( iodirec_b ) );
	_wire->write( highByte( iodirec_b ) );
	_wire->endTransmission( );
	_wire->beginTransmission( (int)i2caddr );
	_wire->write( GPIOA );	//point register pointer to gpio reg
	_wire->write( lowByte(iodirec_b) ); // make o/p latch agree with pulled-up pins
	_wire->write( highByte(iodirec_b) );
	_wire->endTransmission( );
} // _begin( )

// individual pin setup - modify pin bit in IODIR reg.
void Keypad_MC17b::pin_mode(byte pinNum, byte mode) {
...
void Keypad_MC17b::pin_write(byte pinNum, boolean level) {
	word mask = 1<<pinNum;
	if( level == HIGH ) {
		pinState |= mask;
	} else {
...
int Keypad_MC17b::pin_read(byte pinNum) {
	_wire->beginTransmission((int)i2caddr);
	_wire->write( GPIOA );
	_wire->endTransmission( );
...
void Keypad_MC17b::port_write( word i2cportval ) {
...
word Keypad_MC17b::pinState_set( ) {
	_wire->beginTransmission((int)i2caddr);
	_wire->write( GPIOA );
	_wire->endTransmission( );
	_wire->requestFrom( (int)i2caddr, 2 );
...
word Keypad_MC17b::iodir_read( ) {
	return iodir_state;  // local copy is always same as chip's register
} // iodir_read( )

void Keypad_MC17b::iodir_write( word iodir ) {
	iodir_state = iodir;
